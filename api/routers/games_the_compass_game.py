...
async def start(req: StartReq, db: AsyncSession = Depends(get_async_db)):
    ...
    initial_ai = {}
    if req.ai_enabled:
        resp = await safe_chat_completion(system_prompt, "Start journey at The Crossroads.", max_tokens=300)
        try: initial_ai = json.loads(resp)
        except: initial_ai = {"narrative": "You stand at a crossroads...", "location_name": "The Crossroads", "choices": ["North", "South"]}
...
async def action(req: ActionReq, db: AsyncSession = Depends(get_async_db)):
    ...
    journal = current_state.get("compass_journal", [])
    ...
    if current_state.get("ai_enabled"):
        ...
        if player_data.get("truth_mismatch"):
             observer_note = f"\n[SHADOW OBSERVER NOTE]: {player_data.get('last_commentary')} The player's choice here might be performative or contradictory to their nature. Mention this in the narrative or or give them a specific 'Inconsistency Insight'."

        context = f"Current Location: {current_state.get('location')}\nPlayer Choice: {req.content}{observer_note}"
        resp_str = await safe_chat_completion(system_prompt, context, max_tokens=400)
        try:
             ai_response = json.loads(resp_str)
        except:
             ai_response = {"narrative": "You move forward.", "location_name": "Unknown Path"}
