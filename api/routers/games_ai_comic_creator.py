...
@router.post('/chat')
async def chat(req: ChatReq, db: AsyncSession = Depends(get_async_db)):
    gm = GameManager(db)
    session = await gm.get_session(req.session_id)
    ...
    updated_session = await gm.update_state(req.session_id, {"chat_messages": messages})
